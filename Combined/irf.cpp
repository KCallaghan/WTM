...
///This function initialises those arrays that are needed only for transient model runs. 
///This includes both start and end states for slope, precipitation, temperature, 
///topography, ET, and relative humidity. We also have a land vs ocean mask for the end time. 
///It also includes the starting water table depth array, a requirement for transient runs.
///We also calculate the e-folding depth here, using temperature and slope. 
///TODO: the e-folding depth uses some calibration constants that are dependent on cell-size. How to deal with this when a user may
...
///This function initialises those arrays that are needed only for equilibrium model runs. 
...
///It also includes setting the starting water table depth array to zero everywhere.
///We also calculate the e-folding depth here, using temperature and slope. 
///TODO: the e-folding depth uses some calibration constants that are dependent on cell-size. How to deal with this when a user may
///have differing cell size inputs? Should these be user-set values?
...
///Here, we use the number of cells per degree (a user-defined value), the southern-most 
///latitude of the domain (also user-defined), and the radius of the Earth
///to calculate the latitude of each row of cells, the size of a cell in the N-S and E-W 
///directions, and the area of each cell. 
...
void InitialiseBoth(const Parameters &params, ArrayPack &arp){
  ...
  arp.evap               = rd::Array2D<float>(arp.ksat,0);        //Several arrays that are used for calculations of evaporation
  ...
  arp.label              = rd::Array2D<dh_label_t>   (params.ncells_x, params.ncells_y, dh::NO_DEP ); //No cells are part of a depression
  arp.final_label        = rd::Array2D<dh_label_t>   (params.ncells_x, params.ncells_y, dh::NO_DEP ); //No cells are part of a depression
  arp.flowdirs           = rd::Array2D<rd::flowdir_t>(params.ncells_x, params.ncells_y, rd::NO_FLOW); //No cells flow anywhere
...
//Label the ocean cells. This is a precondition for using `GetDepressionHierarchy()`.
  #pragma omp parallel for
...
///In transient runs, we adjust the input arrays via a linear interpolation from the start
///state to the end state at each iteration. We do so here, and also reset
///the label and flow direction arrays, since the depression hierarchy needs to be 
///recalculated due to the changed topography. 
void UpdateTransientArrays(const Parameters &params, ArrayPack &arp){
  for(unsigned int i=0;i<arp.topo.size();i++){

    arp.fdepth(i)         = (arp.fdepth_start(i)        * (1-(params.cycles_done/params.total_cycles))) + (arp.fdepth_end(i)        * (params.cycles_done/params.total_cycles));
    arp.precip(i)         = (arp.precip_start(i)        * (1-(params.cycles_done/params.total_cycles))) + (arp.precip_end(i)        * (params.cycles_done/params.total_cycles));
    arp.temp(i)           = (arp.temp_start(i)          * (1-(params.cycles_done/params.total_cycles))) + (arp.temp_end(i)          * (params.cycles_done/params.total_cycles));
    arp.topo(i)           = (arp.topo_start(i)          * (1-(params.cycles_done/params.total_cycles))) + (arp.topo_end(i)          * (params.cycles_done/params.total_cycles));
    arp.starting_evap(i)  = (arp.starting_evap_start(i) * (1-(params.cycles_done/params.total_cycles))) + (arp.starting_evap_end(i) * (params.cycles_done/params.total_cycles));
...
///In this function, we use a few of the variables that were created for informational purposes
///to help us understand how much the water table is changing per iteration, and where in 
...
void PrintValues(Parameters &params, ArrayPack &arp){
  ...
  for(int x=1;x<params.ncells_x-1; x++){
    ...
    params.evaporation_change   += arp.evaporation_array(x,y);
  ...
  textfile<<"total wtd change was "<<params.total_wtd_change<<" change in GW only was "<<params.GW_wtd_change<<" and change in SW only was "<<params.wtd_mid_change<<std::endl;
  textfile<<"absolute value total wtd change was "<<params.abs_total_wtd_change<<" change in GW only was "<<params.abs_GW_wtd_change<<" and change in SW only was "<<params.abs_wtd_mid_change<<std::endl;
  textfile<<"the change in infiltration was "<<params.infiltration_change<<" change in evaporation was "<<params.evaporation_change<<" and change to surface water was "<<params.surface_change<<std::endl;
  textfile.close();
}