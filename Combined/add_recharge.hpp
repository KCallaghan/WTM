...
void add_recharge(const int x, const int y, Parameters &params, ArrayPack &arp){
  ...
  if(arp.wtd(x,y) >= 0)  //all the recharge will occur above the land surface; don't worry about porosity. 
    ...
    if(arp.wtd(x,y) > 0){ //part of the change is below the ground, part above. So we need to calculate how much of the water was used up in the ground, i.e. the portion between the starting wtd and 0.
