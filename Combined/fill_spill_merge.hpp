...
static void FillAFullDepression(
  ...
  int cells_changed = 0;
  for(const auto c: stdi.my_labels){
    for(const auto d: deps.at(c).my_cells){
      if(arp.topo(d)<deps.at(stdi.top_label).out_elev)
        arp.wtd(d) = deps.at(stdi.top_label).out_elev - arp.topo(d);
    }
  }
}
...
static void FillDepressions(
  ...
  assert(fp_le(deps.at(stdi.top_label).water_vol, deps.at(stdi.top_label).wtd_vol) );
...
//if(arp.wtd(c.x,c.y)>0.000000001)
 // std::cout<<"wtd is positive "<<arp.wtd(c.x,c.y)<<" label "<<stdi.top_label<<std::endl;
...
//    if(water_vol - (current_volume - (arp.cell_area[c.y] * arp.porosity(c.x,c.y) *
  //    arp.fdepth(c.x,c.y) * (exp(arp.wtd(c.x,c.y)/arp.fdepth(c.x,c.y)) - 1) ) ) <= FP_ERROR){
    ...
    //temp = (static_cast<double>(arp.cell_area[c.y]) * static_cast<double>(arp.porosity(c.x,c.y)) * static_cast<double>(arp.wtd(c.x,c.y)) ) / 1000.;

    if( (((water_vol - (current_volume - (static_cast<double>(arp.cell_area[c.y]) * static_cast<double>(arp.porosity(c.x,c.y)) * static_cast<double>(arp.wtd(c.x,c.y)) ) ) )<=FP_ERROR) && stdi.my_labels.count(arp.label(c.x,c.y))==1) || water_vol-current_volume < FP_ERROR ) {
