...
double updateEffectiveStorativity(const double my_original_wtd, const double my_wtd_T, const double my_porosity, const double starting_effective_storativity){
  ...
  if(my_original_wtd <= 0. && projected_full_step_wtd <= 0.) //both are below ground, so we can use the original porosity
    ...
    if(my_original_wtd < 0. && projected_full_step_wtd > 0.){ //started below ground and ended above ground
      ...
      const double scaled_change_in_water_column_thickness = change_in_water_column_thickness * starting_effective_storativity/static_cast<double>(my_porosity);
      ...
      const double aboveground_water_column_thickness = (scaled_change_in_water_column_thickness + my_original_wtd) * starting_effective_storativity;//Above-ground effective porosity is ==1, so no need to actually divide by 1 here (save on computation).
      ...
      const double scaled_change_in_water_column_thickness = change_in_water_column_thickness * starting_effective_storativity; //divided by 1 for above-ground porosity
      if(scaled_change_in_water_column_thickness < projected_full_step_wtd){  //when rescaling the water according to the porosity values, there is no longer enough to reach below ground; so the scaled new porosity will be = 1
        ...
        const double belowground_water_column_thickness = (scaled_change_in_water_column_thickness - my_original_wtd) * starting_effective_storativity/static_cast<double>(my_porosity);
