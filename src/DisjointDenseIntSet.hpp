...
class DisjointDenseIntSet {
 ...
 public:
  ...
  void makeSet(unsigned int n) { checkSize(n); }

  // Returns the highest set id.
  unsigned int maxElement() const { return rank.size() - 1; }
  ...
  bool sameSet(unsigned int a, unsigned int b) { return findSet(a) == findSet(b); }
};
