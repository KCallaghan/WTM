...
inline double add_recharge(const double deltat, const double my_rech, double my_wtd, const double my_porosity) {
  ...
  } else if (rech_change > 0) {  // at least some of the water will be added into the ground, so we need to think about
                                 // porosity. If rech_change was < 0 we don't add it here since there is no surface
